...
class AuditReport(BaseModel):
    ...
    Diagnostic analysis of owned (self-published) notes to identify:
    - Metric weaknesses (which prediction metrics are underperforming)
    - Content gaps (missing or weak features compared to best practices)
    ...
    current_metrics: Dict[str, float] = Field(
        description="当前笔记的全部预测指标 (All 10 prediction metrics: ctr, comment_rate, etc.)"
    )
    ...
    # Metric analysis
    weak_metrics: List[str] = Field(
        description="表现不佳的指标列表 (Underperforming metrics, e.g., ['ctr', 'sort_score2'])",
        default_factory=list
    )

    strong_metrics: List[str] = Field(
        description="表现优秀的指标列表 (High-performing metrics)",
        default_factory=list
    )

    # Content analysis
    content_weaknesses: List[str] = Field(
        description="内容弱点列表 (Content gaps or weak features, e.g., 'Missing emotional hook in title')"
    )

    content_strengths: List[str] = Field(
        description="内容优势列表 (Strong features already present)",
        default_factory=list
    )

    # Overall diagnosis
    overall_diagnosis: str = Field(
        description="整体诊断摘要 (Concise summary of key issues, 50-200 chars)"
    )

    # Metadata
    audit_timestamp: str = Field(
        description="审计时间戳 (ISO 8601 format, e.g., '2025-11-19T10:30:00Z')"
    )

    @field_validator('current_metrics')
    @classmethod
    def validate_current_metrics_not_empty(cls, v: Dict[str, float]) -> Dict[str, float]:
        """Validate current_metrics dict is not empty."""
        if len(v) == 0:
            raise ValueError("current_metrics cannot be empty")
        return v

    @field_validator('content_weaknesses')
    @classmethod
    def validate_content_weaknesses_not_empty(cls, v: List[str]) -> List[str]:
        """Validate content_weaknesses list is not empty (there should always be room for improvement)."""
        if len(v) == 0:
            raise ValueError("content_weaknesses cannot be empty - every note has room for improvement")
        return v

    @field_validator('overall_diagnosis')
    @classmethod
    def validate_diagnosis_length(cls, v: str) -> str:
        """Validate overall_diagnosis is substantial (50-200 chars)."""
        if not (50 <= len(v) <= 200):
            raise ValueError(f"overall_diagnosis must be 50-200 characters, got {len(v)}")
        return v
